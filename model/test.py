...
ckpt = torch.load("runs/fcos_custom/fcos_epoch50.pth", map_location="cpu")
...
with torch.no_grad():
    ...
    for i, (cls, cent, box) in enumerate(zip(classes, centerness, boxes)):
        # Get the actual class probabilities (before sigmoid)
        cls_raw = cls.view(-1, cls.shape[-1])  # Flatten spatial dims
...
# 4. Use debug detection function instead of normal one
H, W = 512, 512
print("\n" + "="*50)
print("CALLING DEBUG DETECTION FUNCTION")
print("="*50)

# Replace detections_from_network_output with debug version
...
if len(detections) > 0:
    for i, det in enumerate(detections):
        print(f"Detection {i}: class={det.object_class}, score={det.score:.4f}, bbox={det.bbox}")
